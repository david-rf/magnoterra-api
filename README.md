...
- `POST /api/webhooks` - Webhook genérico para `event=youtube_upload_batch`
- `POST /api/webhooks/youtube-upload-batch` - Webhook directo de videos YouTube

### Webhook YouTube

Enviar JSON con `event=youtube_upload_batch` y `videos[]` con `video_id`,
`url` y `job`. La respuesta es `text/markdown` con URL, copy LinkedIn empresa
y caption Instagram por video. Si el payload no incluye videos, responde
`NO_VIDEOS`.
