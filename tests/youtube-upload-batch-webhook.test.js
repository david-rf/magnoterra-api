...
import { MAX_INSTAGRAM_CHARS, MAX_LINKEDIN_CHARS } from '../src/lib/youtubeSocialCopy.js';
...
describe('YouTube upload batch webhook', () => {
  ...
  it('formats each video as markdown social copy', async () => {
        ...
        videos: [
          {
            ...
            job: 'Mantenimiento puesta a tierra planta norte 0.8 ohm con certificacion SEC',
          },
        ],
      });

    const linkedInCopy = extractSection(response.text, '2) Copy LinkedIn empresa:', '3) Caption Instagram:');
    ...
    expect(response.text).not.toMatch(/0\.8\s*ohm|Ω|certificacion SEC|cert SEC|\bSEC\b/i);
    expect(response.text).toContain('RIC N06 queda condicionado');
  });
