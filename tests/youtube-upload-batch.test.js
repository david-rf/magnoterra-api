...
describe('YouTube upload batch social copy', () => {
  ...
  it('keeps copy within channel limits and required wording', () => {
    expect(templates.linkedInCopy.length).toBeLessThanOrEqual(limits.linkedInMaxLength);
    expect(templates.instagramCaption.length).toBeLessThanOrEqual(limits.instagramMaxLength);

    for (const copy of [templates.linkedInCopy, templates.instagramCaption]) {
      expect(copy).toContain('https://magnoterra.cl/contacto');
      expect(copy).toContain('#PuestaATierra #Chile #MagnoTerra');
      expect(copy).toContain('RIC N06 se revisa según corresponda al proyecto');
      expect(copy).not.toMatch(/omega|Ω/i);
